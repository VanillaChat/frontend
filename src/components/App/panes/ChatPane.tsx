...
const ChatPane: React.FC = () => {
  ...
  const handleScroll = useCallback(() => {
    if (!msgRef.current || isInitialLoad.current || isLoadingRef.current) return;
    ...
    if (scrollTop < 100 && scrollDirection === 'up' && !messages.isLoadingMore && messages.hasMoreMessages[channelId!]) {
      isLoadingRef.current = true;
      isViewingOlderMessages.current = true;
      setPrevScrollHeight(scrollHeight);

      messages.loadMoreMessages(channelId!).finally(() => {
        isLoadingRef.current = false;
        const channelMessages = messages.data[channelId!] || [];
        if (channelMessages.length > 50) {
          messages.trimMessages(channelId!, 'up');
        }
      });
    } else if (scrollBottom < 100 && scrollDirection === 'down' && !messages.isLoadingNewer && messages.hasNewerMessages[channelId!]) {
      isLoadingRef.current = true;
      isViewingOlderMessages.current = false;

      messages.loadNewerMessages(channelId!).finally(() => {
        isLoadingRef.current = false;
        const channelMessages = messages.data[channelId!] || [];
        if (channelMessages.length > 50) {
          messages.trimMessages(channelId!, 'down');
        }
      });
    }
  ...
  useEffect(() => {
    ...
    if (messages.isLoadingMore && prevScrollHeight > 0) {
      ...
      return () => {
        ...
        scrollContainer.scrollTop = newScrollHeight - oldScrollHeight + scrollContainer.scrollTop;
