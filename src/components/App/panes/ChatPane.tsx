		...
		<div className="w-full h-full flex flex-col dark:bg-[#262622] dim:bg-[#141413]">
			<div className="bg-[#FBFBFB] dark:bg-[#36362E] dim:bg-[#171717] dark:border-b-[#36362E] dim:border-b-[#171717] h-[52px] p-[20px_25px] flex flex-row border-b-[1px] border-b-[#e0e0e0] items-center gap-[5px]">
				...
				<p className="text-[16px] font-medium">
					{isDM ? "Unknown User" : "Unknown Channel"}
				</p>
			</div>
			<Input
...
const ChatPane: React.FC = () => {
	...
	const channels = useChannels(
		(state) =>
			state.data[
				isDM ? (session.currentUser?.id as string) : (guildId as string)
			],
	);
	...
	const handleScroll = useCallback(() => {
		if (!msgRef.current || isInitialLoad.current || isLoadingRef.current)
			return;
		...
		if (
			...
			messages.hasMoreMessages[channelId as string]
		) {
			isLoadingRef.current = true;
			isViewingOlderMessages.current = true;
			setPrevScrollHeight(scrollHeight);

			messages.loadMoreMessages(channelId as string).finally(() => {
				isLoadingRef.current = false;
				const channelMessages = messages.data[channelId as string] || [];
				if (channelMessages.length > 50) {
					messages.trimMessages(channelId as string, "up");
				}
			});
		} else if (
			scrollBottom < 100 &&
			...
			messages.hasNewerMessages[channelId as string]
		) {
			isLoadingRef.current = true;
			isViewingOlderMessages.current = false;

			messages.loadNewerMessages(channelId as string).finally(() => {
				isLoadingRef.current = false;
				const channelMessages = messages.data[channelId as string] || [];
				if (channelMessages.length > 50) {
					messages.trimMessages(channelId as string, "down");
				}
			});
		}
	}, [channelId, messages]);
	...
	useEffect(() => {
		...
		if (typeof savedContent[channelId] === "undefined")
			setContent(channelId, "");
		if (typeof data[channelId] === "undefined") {
			setMessages(
				channelId,
				loadedMessages.map((msg: any) => ({ ...msg, state: "SENT" })),
			);
		...
		if (messages.isLoadingMore && prevScrollHeight > 0) {
			...
			return () => {
				...
				scrollContainer.scrollTop =
					newScrollHeight - oldScrollHeight + scrollContainer.scrollTop;
	...
	useEffect(() => {
		...
		channelId,
		messages.data[channelId as string],
		messages.isLoadingMore,
	...
	return (
		<div className="w-full h-full flex flex-col min-h-0 dark:bg-[#262622] dim:bg-[#141413]">
			<div className="bg-[#FBFBFB] dark:bg-[#36362E] dim:bg-[#171717] dark:border-b-[#36362E] dim:border-b-[#171717] h-[52px] p-[20px_25px] flex flex-row border-b-[1px] border-b-[#e0e0e0] items-center gap-[5px]">
				<h1 className="text-[24px] font-normal m-0">{isDM ? "@" : "#"}</h1>
				<p className="text-[16px] font-medium">
					{isDM ? "John Doe" : channel?.name}
				</p>
			</div>
			<ul
				className="max-w-[100%] flex-1 min-h-0 pl-0 p-[10px] m-0 flex flex-col justify-start items-start min-w-0 dark:bg-[#262622] dim:bg-[#141413] list-none overflow-y-auto"
				ref={msgRef}
			>
				{messages.isLoadingMore && (
					<>
				...
				{messages.data[channelId as string]?.length > 0 &&
					Array.from(messages.data[channelId as string]).map(
						(message, index) => (
							<Message
				...
				{(messages.data[channelId as string]?.length ?? 0) === 0 && (
			...
			{messages.typingIndicators[channelId as string]?.length > 0 && (
				<div className="mb-2 items-center gap-2 rounded-[8px] py-[4px] px-[10px] transition-all duration-[.2s] focus:border-[#dbddd0] dark:bg-[#393830] dark:border-[#464540] dark:text-white dim:bg-[#181815] dim:border-[#302F2A] dim:text-white border-[1px] border-[#D3D2C8] bg-[#fffefa] w-[98%] flex self-center">
					...
					<small className="text-[12px]">
						{t("app.chat.typing", {
							user1:
								messages.typingIndicators[channelId as string][0]?.username,
							user2:
								messages.typingIndicators[channelId as string][1]?.username,
							count: messages.typingIndicators[channelId as string].length,
							remainingCount: Math.max(
								0,
								messages.typingIndicators[channelId as string].length - 2,
							),
						})}
					</small>
				</div>
					...
					<button
						...
						onClick={async () => {
							if (msgRef.current) {
								...
								await messages.loadNewerMessages(channelId as string);
							}
						}}
						type="button"
					>
