...
import ChannelLink from "../../UI/ChannelLink";
...
const ChannelSidebar: React.FC = () => {
	...
	const channels = useChannels((state) => state.data[guildId as string]);
	...
	useMeta(
		`#${channels?.find((channel) => channel.id === channelId)?.name} - ${server?.name}`,
	);

	if (server) {
		return (
			<div className="w-[300px] bg-[#f2f2f2] dark:bg-[#2C2B27] dim:bg-[#070707] p-[20px_10px]">
				<div className="bg-[#fbfbfb] justify-between border-[1px] border-[#e0e0e0] dark:bg-[#393830] dark:border-[#464540] dim:bg-[#181815] dim:border-[#302F2A] rounded-[8px] mt-[-10px] flex flex-row p-[15px_15px] items-center">
					<div>
						<p className="text-[16px] font-medium mb-[5px]">{server.name}</p>
						<p className="text-[14px] font-normal mt-0">{server?.brief}</p>
					</div>
					<Menu.Root>
						<Menu.Trigger>
							<svg
								className="w-[20px] h-[20px] ml-auto text-black dark:text-white dim:text-white opacity-70 hover:opacity-100 cursor-pointer"
								width="18"
								height="18"
								viewBox="0 0 18 18"
								fill="none"
						...
						<ChannelLink
							...
							onInviteClick={async () => {
								const res = await fetch(
									`${import.meta.env.VITE_API_URL}/channels/${channel.id}/invites`,
									{
										method: "POST",
										body: JSON.stringify({}),
										headers: {
								...
								if (res.status === 200) {
									...
									alert(`${import.meta.env.VITE_INVITE_URL}${json.code}`);
								}
							}}
						/>
					))}
				...
				<Modal
					show={showLeaveServerModal}
					close={() => setShowLeaveServerModal(false)}
					title={`Leave ${server.name}`}
					confirmText="Leave"
					subtitle={`Do you really want to leave ${server.name}? You won't be able to rejoin unless you are re-invited.`}
					onConfirm={async () => {
						const res = await fetch(
							`${import.meta.env.VITE_API_URL}/users/@me/guilds/${guildId}`,
							{
								method: "DELETE",
								credentials: "include",
							},
						...
						if (res.status === 204) {
							...
							servers.delete(guildId as string);
							messages.clearServer(channels.map((ch) => ch.id));
							channelStore.clear(guildId as string);
						}
					}}
				></Modal>
			</div>
	...
	return (
		...
		>
			<path
				...
				className="fill-[canvas] dim:fill-black dark:fill-[#2C2B27]"
			/>
			<path
