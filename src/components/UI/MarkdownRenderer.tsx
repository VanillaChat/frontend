...
export const MarkdownRenderer: FC<MarkdownRendererProps> = ({
		...
		while (i < lines.length) {
			...
			if (line.startsWith("```") && !disabledFeatures?.includes("codeblock")) {
				...
				result.push(
					<ShikiHighlighter
						...
						theme={theme === "light" ? "one-light" : "one-dark-pro"}
			...
			if (headerMatch) {
				...
				result.push(
					<Tag
						...
						style={{ fontSize: { 1: "42px", 2: "26px", 3: "18px" }[level] }}
					>
						{parseInline(content)}
					</Tag>,
				);
			...
			if (line.startsWith(">")) {
				...
				result.push(
					<blockquote
						key={`quote-${i}`}
						className="border-l-[#ccc] border-l-[3px] rounded-2 pl-[0.5em] select-text"
					>
			...
			result.push(
				<p key={`p-${i}`} className="select-text">
					{parseInline(line)}
				</p>,
			);
			i++;
	...
	const tokens: {
	...
	}[] = [
		{
			regex: /~~(.+?)~~/,
			render: ([, content], ctx) => (
				<del key={nanoid()}>{parseInline(content, ctx)}</del>
			),
		},
		{
			regex: /\*\*\*(.+?)\*\*\*/,
		...
		},
		{
			regex: /\*\*(.+?)\*\*/,
			render: ([, content], ctx) => (
				<strong key={nanoid()}>{parseInline(content, ctx)}</strong>
			),
		},
		{
			regex:
				/(?<!\w)_(?!_)(.+?)(?<!_)_(?!\w)|(?<!\w)\*(?!\*)(.+?)(?<!\*)\*(?!\w)/,
			render: (m, ctx) => (
				<em key={nanoid()}>{parseInline(m[1] || m[2], ctx)}</em>
			),
		},
		{
			regex: /`([^`]+)`/,
		...
		while (remaining) {
			...
			if (!matched) {
				const nextMatchIndex = tokens
					...
					.reduce(
						(min, i) => Math.min(min as number, i as number),
						remaining.length,
					);

				const literal = remaining.slice(0, nextMatchIndex);
				const node = ctx.currentColor ? (
					<span style={{ color: ctx.currentColor }} key={nanoid()}>
						{literal}
					</span>
				) : (
					literal
