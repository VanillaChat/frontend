...
import { type InviteData, useInvites } from "@/store/invites";
...
const MessageWithInvites: React.FC<MessageWithInvitesProps> = ({ content }) => {
	...
	const checkMembershipStatus = React.useCallback(
	...
	useEffect(() => {
		const fetchInvites = async () => {
			...
			for (const code of inviteCodes) {
				...
				let inviteData: InviteData | undefined | null;
		...
		invites.fetchInvite,
		invites.getInvite,
		invites.getMembershipStatus,
		invites.hasFetchError,
		invites.hasInvite,
		invites.hasMembershipStatus,
		invites.setMembershipStatus,
	]);
	...
	return (
		<div className="flex flex-col border rounded-md p-3 my-2 max-w-[400px] bg-[#f0f0e8] dark:bg-[#49473f] dim:bg-[#282828] border-[#D3D2C8] dark:border-[#464540] dim:border-[#302F2A]">
			...
			<p className="text-sm mt-1 text-gray-600 dark:text-gray-300 dim:text-gray-400">
				The invite information could not be loaded. The invite may be invalid or
				expired.
			</p>
		</div>
	);
