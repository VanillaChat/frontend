					...
					>
						...
						<div className="w-[100%] h-[500px] rounded-lg p-1">
							<div
								...
								style={{
									backgroundImage: props.user.banner
										? `url(${import.meta.env.VITE_API_URL}/cdn/banners/${props.user.id}/${props.user.banner}.webp)`
										: "none",
								}}
							>
								<div className="border-white bg-white dark:border-[#262622] dark:bg-[#262622] dim:border-black dim:bg-black border-[2px] translate-y-[80px] translate-x-[15px] w-fit rounded-[100%]">
									<Avatar
										...
										style={{
											border:
												`${props.user.status === "UNAVAILABLE" ? "0" : "4"}px solid ` +
												{
							...
							<div className="px-5 flex flex-col mt-[30px] h-[calc(100%-160px)]">
								<div className="flex flex-row gap-1 items-center">
									...
									<h1 className="text-[18px] font-semibold opacity-80">
										{props.user.tag}
									</h1>
								</div>
								...
								<div className="flex flex-col mt-[15px]">
									...
									<p className="text-[14px] items-center select-text flex flex-row gap-2">
										{props.user.id}
										<button
											className={cn(buttonStyles({ filled: false }), "p-1.5")}
											onClick={() =>
								...
								<div className="flex flex-col gap-1 justify-end h-[100%] mb-4">
									...
									<Select.Root
										...
										onValueChange={(value) => {
											...
											Object.entries(members.data).forEach(
												([guildId, guildMembers]) => {
...
function ChevronUpDownIcon(props: React.ComponentProps<"svg">) {
	return (
		<svg
			...
			strokeWidth="1.5"
			role="img"
			aria-label="chevron up down icon"
			{...props}
		>
