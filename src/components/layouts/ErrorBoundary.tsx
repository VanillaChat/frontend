...
export default function ErrorBoundary() {
    ...
    if (isRouteErrorResponse(error)) {
        ...
        err = 'We really don’t know, why you’re here... It’s a weird place, perhaps you’re lost?';
        stack = undefined;
    }
    // if (isRouteErrorResponse(error)) {
    //     return <>
        ...
        >
            ...
            {stack &&
                <Collapsible title="Expand error" expandedTitle="Collapse error">
                    <Highlight language="javascript" code={stack} theme={themes.oneLight}>
                        {({ style, tokens, getLineProps, getTokenProps }) => (
                            <pre style={{...style, userSelect: 'text', pointerEvents: 'all'}}>
                                {tokens.map((line, i) => (
                                    <div key={i} style={{userSelect: 'text', pointerEvents: 'all'}} {...getLineProps({ line })}>
                                        {line.map((token, key) => (
