...
export default function Login() {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof LoginSchema>> = useCallback(async (data) => {
  ...
  return (
      <main id="login" className="flex flex-row justify-center dark:bg-[#262622] dim:bg-black gap-[90px] mt-[10vh] pb-[20px] max-[850px]:flex-col max-[850px]:justify-center max-[850px]:items-center max-[850px]:gap-0 max-[850px]:mt-[5vh]">
        <div className="max-[850px]:flex max-[850px]:flex-col max-[850px]:justify-center max-[850px]:items-center">
          ...
          <h1 className="text-[30px] font-semibold mb-0 text-[#101028] mt-[20.1px] dark:text-white dim:text-white">{t('login.title')}</h1>
          <p className="my-[16px] text-[#667085] dark:text-white dim:text-white">{t('login.subtitle')}</p>
        </div>
        <div className="w-[385px]">
          {globalError && <Alert variant="destructive" title="Validation error">{t(globalError)}</Alert>}
          <form className="mt-[10px]" onSubmit={handleSubmit(onSubmit)} autoComplete="off">
            <FormInput
              id="email"
              name="email"
