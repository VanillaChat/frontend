	...
	return (
		...
		>
			...
			<div className="w-[385px]">
				...
				<form className="mt-[10px]" onSubmit={handleSubmit(onSubmit)}>
					<FormInput
						...
						translatedError={{
							...
							params: { minLength: 2, maxLength: 64 },
						}}
					...
					{import.meta.env.VITE_REGISTRATION_CLOSED && (
						<FormInput
							...
							name="inviteCode"
							id={`inviteCode-${nanoid()}`}
							labelStyle={{ marginTop: "15px" }}
							register={register}
							translatedError={{ error: errors.inviteCode as FieldError }}
							// onChange={(e) => setData(d => ({...d, email: e.target.value}))}
						/>
					)}
					<FormInput
						...
						translatedError={{
							...
							params: { minLength: 8, maxLength: 128 },
						}}
					...
					<FormInput
						...
						id={`confirmPassword-${nanoid()}`}
						register={register}
						translatedError={{ error: errors.confirmPassword as FieldError }}
				...
				<Button style={{ width: "100%", marginTop: "20px" }} icon={DiscordIcon}>
					{t("register.useDiscord")}
				</Button>
				<p className="text-center mt-7 text-[#667085] dark:text-white dim:text-white">
					{t("register.alreadyHaveAccount")}{" "}
					<Link to="/login" className="text-[#D8BC21] no-underline">
						{t("register.signIn")}
					</Link>
				</p>
			</div>
