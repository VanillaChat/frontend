...
export type MessageState = {
	...
	clearServer: (channelIds: string[]) => void;
...
export const useMessages = create<MessageState>()(
	devtools((set, get) => ({
		...
		addTypingIndicator: (channel: string, user: User) =>
			set((state) => {
				const channelTypingIndicators = state.typingIndicators[channel] || [];

				const userAlreadyTyping = channelTypingIndicators.some(
				...
				if (!userAlreadyTyping) {
					return {
						typingIndicators: {
							...
							[channel]: [...channelTypingIndicators, user],
						},
					};
				}
		...
		removeTypingIndicator: (channel: string, user: string) =>
			set((state) => {
				const channelTypingIndicators = state.typingIndicators[channel] || [];
		...
		pushOptimistic: (channel: string, message: Message) => {
			...
			set((state) => ({
				data: {
					...
					[channel]: [
						...(state.data[channel] || []),
						{ ...message, id: tempId, state: "SENDING" },
					],
				},
			}));
			...
			return {
				edit: (patch: Partial<Message>) =>
					get().updateMessage(channel, tempId, patch),
		...
		updateMessage: (channel, id, message) =>
			set((state) => ({
				data: {
					...state.data,
					[channel]: (state.data[channel] || []).map((msg) =>
		...
		setContent: (channel: string, content: string) =>
			set((state) => ({
				savedContent: { ...state.savedContent, [channel]: content },
			})),
		setHasMoreMessages: (channel: string, hasMore: boolean) =>
			set((state) => ({
				hasMoreMessages: { ...state.hasMoreMessages, [channel]: hasMore },
			})),
		setHasNewerMessages: (channel: string, hasNewer: boolean) =>
			set((state) => ({
		...
		trimMessages: (channel: string, direction = "none") => {
			...
			const buffer = Math.min(
				20,
				Math.floor(state.maxMessagesPerChannel * 0.2),
			);
		...
		loadMoreMessages: async (channel: string) => {
			...
			if (
				oldestMessageId &&
				state.lastLoadedOldestMessageId[channel] === oldestMessageId
			) {
				set((state) => ({
					hasMoreMessages: {
			...
			try {
				...
				if (!response.ok) {
					console.log(
						`Failed to fetch older messages: ${response.status} ${response.statusText}`,
					);
					return;
				}
				...
				set((state) => ({
					data: {
						...state.data,
						[channel]: [
							...olderMessages.map((msg: any) => ({ ...msg, state: "SENT" })),
							...state.data[channel],
						],
					},
					hasMoreMessages: {
		...
		loadNewerMessages: async (channel: string) => {
			...
			const newestMessageId =
				state.data[channel]?.[state.data[channel].length - 1]?.id;
			if (
				newestMessageId &&
				state.lastLoadedNewestMessageId[channel] === newestMessageId
			) {
				set((state) => ({
					hasNewerMessages: {
			...
			try {
				...
				if (!response.ok) {
					console.log(
						`Failed to fetch newer messages: ${response.status} ${response.statusText}`,
					);
					return;
				}
				...
				set((state) => ({
					data: {
						...
						[channel]: [
							...state.data[channel],
							...newerMessages.map((msg: any) => ({ ...msg, state: "SENT" })),
						],
					},
					hasNewerMessages: {
						...state.hasNewerMessages,
		...
		clearServer(channelIds: string[]) {
			set((state) => {
				const newState = { ...state };

				channelIds.forEach((channelId) => {
					delete newState.data[channelId];
					delete newState.savedContent[channelId];
					delete newState.typingIndicators[channelId];
					delete newState.hasMoreMessages[channelId];
					delete newState.hasNewerMessages[channelId];
					delete newState.lastLoadedOldestMessageId[channelId];
					delete newState.lastLoadedNewestMessageId[channelId];
				});
...
export const useEditCache = create<EditCacheState>()(
	devtools((set) => ({
		cache: {
			isEditing: false,
			messageId: null,
		...
		clear: () =>
			set(() => ({
				cache: { isEditing: false, messageId: null, content: null },
			})),
	})),
);
