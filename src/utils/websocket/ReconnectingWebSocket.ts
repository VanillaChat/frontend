...
/**
 * Returns true if the given argument looks like a WebSocket class
 */
...
export default class ReconnectingWebSocket {
	...
	set heartbeatInterval(value: NodeJS.Timeout | null) {
	...
	public removeEventListener<T extends keyof Events.WebSocketEventListenerMap>(
		...
		if (this._listeners[type]) {
			...
			this._listeners[type] = this._listeners[type].filter(
				// @ts-expect-error
				(l: Events.WebSocketEventListenerMap[T]) => l !== listener,
			);
		}
	}
		...
		this._wait()
			...
			.then((url) => {
				...
				this._ws = this._protocols
					? new WebSocket(url, this._protocols, this._options)
					: new WebSocket(url, [], this._options);
				(this._ws as WebSocket).binaryType = this._binaryType;
				this._connectLock = false;
				this._addListeners();
